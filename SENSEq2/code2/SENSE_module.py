...
from math import exp, cos, sin, sqrt, asin, acos, atan2, fabs
...
e0 = 8.85e-12             # epsilon_0 [C^2/Nm^2]
...
#-----------------------------------------------------
def dE2(Nout,Ntot,ct,phi,omega,xi,ax,axi,axi2,c1,betaz):
    nLambda = Nout - 1
    omLam = omega/nLambda
    st = sqrt(1.0-ct**2)
    cp = cos(phi)
    sp = sin(phi)
    c2 = (1.0 - betaz * ct)/(1 + betaz)
    c3 = st * cp/c1
    c4 = (1.0 + ct)/(2.0*(c1**2))
    c6 = (ct - betaz)/(1 - betaz*ct)

    arg = omLam * (c2*np.array(xi) - c3*np.array(axi) + c4*np.array(axi2))
    ca = np.cos(arg)
    sa = np.sin(arg)
    n1x = np.sum(np.array(ax) * np.array(ca))
    n2x = np.sum(np.array(ax) * np.array(sa))
    n1z = np.sum(np.array(ax**2) * np.array(ca))
    n2z = np.sum(np.array(ax**2) * np.array(sa))

    Dx = sqrt(n1x**2 + n2x**2)/(c1*nLambda)
    Dz = sqrt(n1z**2 + n2z**2)/(2*c1*c1*c2*nLambda)
    Esigma = (omega*Dx*sp)**2
    Epi = (omega*(Dx*c6*cp + Dz*st))**2
    Yval = Epi + Esigma
    return Yval
