...
def readICs(fileICs):
    ...
    for n in range(0,Np):
        ...
        n_g.append(d[n,6])
        n_s.append(d[n,7])
    return pxv, pyv, pzv, gav, xv, yv, n_g, n_s, Np
...
#----------------------------------------------------------------------
def sample_df(Npart,sigma_e_x,sigma_e_y,sigma_e_px,sigma_e_py,gamma,sig_g,pmag0,sigmaPmag,sigma_p_x,sigma_p_y,a0,lm,sign,lambda0):
...
def process_one(xplot, oneSpectrum, gamma, a0, lambda0, ns, Nout, mode):
    ...
    ga = gamma
    f = interp1d(xplot, oneSpectrum, kind='quadratic')
...
if __name__ == '__main__':
    ...
    if (iFile != 1):   # randomly generated from input parameters
        lm = lambda0 / (me * c)
        pxv, pyv, pzv, gav, xv, yv, n_g, n_s, Npart_all = sample_df(Npart,sigma_e_x,sigma_e_y,sigma_e_px,sigma_e_py,gamma,sig_g,pmag0,sigmaPmag,sigma_p_x,sigma_p_y,a0,lm,sign,lambda0)
...
#        yplotNn.append(Yall[i]/Ynorm)       # dN/dE' [1/eV]
    ...
    for i in range(0, Nout):
        ...
        if (RRmode==3):
            ...
            line = r'%15.8e  %15.8e  %15.8e  %15.8e' % (xplot[i], xplotN[i], yplot[i], yplotN[i])
        f.write(line)
        f.write('\n')
    f.close()
