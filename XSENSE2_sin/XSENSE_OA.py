...
#-----------------------------------------------------------------
def readICs(fileICs,lambda0,a0,sign):
    pxv = []
    pyv = []
    pzv = []
    gav = []
    ...
    for n in range(0,Np):
        ...
        xv.append(d[n,4])
        yv.append(d[n,5])
        n_gamma=(2*pi*alpha*lambda0/2*a0**2*sign)/3
        n_g.append(n_gamma)
...
if __name__ == '__main__':
    ...
    if (iFile != 1):   # randomly generated from input parameters
        ...
        pxv, pyv, pzv, gav, xv, yv, n_g, n_s, Npart_all = readICs('data_ICs.in',lambda0,a0,sign)
        Npart=Npart_all
